...
const ConversationBubble: React.FC<ConversationBubbleProps> = ({ message, isUser }) => {
        ...
        <div className={`rounded-2xl px-4 py-2.5 ${bubbleColorClass}`}>
          {message.isAudio ? (
            <div className="h-10 w-32">
              <VoiceVisualizer isActive={false} color={isUser ? 'rgba(255, 255, 255, 0.8)' : 'rgba(59, 130, 246, 0.8)'} />
            </div>
          ) : (
            <p>{message.content}</p>
          )}
